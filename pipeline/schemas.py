...
class Blink(BaseModel):
    ...
    is_anomalous: bool = Field(
        ...,
        description=(
            "True if blink behavior deviates significantly from the speaker's typical blinking pattern"
    ...
    continuous_anomaly: bool = Field(
        ...,
        description="True if this frame is part of a sustained speaking-rate anomaly spanning multiple consecutive frames.",
    )
...
class Gaze(BaseModel):
    ...
    is_anomalous: bool = Field(
        ...,
        description=(
            "True if gaze direction or movement is unusually unstable or extreme over time"
            "based on anomaly detection (Robust Random Cut Forest algorithm used with MAD threshold)."
        ),
    )
...
class Jaw(BaseModel):
    ...
    is_anomalous: bool = Field(
        ...,
        description=(
            "True if jaw movement intensity or pattern deviates from speaker's baseline articulation"
            "based on anomaly detection (Robust Random Cut Forest algorithm used with MAD threshold)."
        ),
    )
...
class Smile(BaseModel):
    ...
    left_intensity: float | None = Field(
        default=None,
        ge=0.0,
        le=1.0,
    ...
    right_intensity: float | None = Field(
        default=None,
        ge=0.0,
        le=1.0,
    ...
    mouth_stretch: float | None = Field(
        default=None,
        ge=0.0,
        le=1.0,
    ...
    is_anomalous: bool = Field(
        ...,
        description=(
            "True if smiling behavior is unusually strong, absent, or unstable for the speaker"
            "based on anomaly detection (Robust Random Cut Forest algorithm used with MAD threshold)."
        ),
    )
...
class LoudnessState(BaseModel):
    level: Literal["very_quiet", "quiet", "normal", "loud", "very_loud"] = Field(
        ...,
        description=(
...
class PitchState(BaseModel):
    relative_level: Literal["much_lower", "lower", "normal", "higher", "much_higher"] = Field(
        ...,
        description=(
...
class PitchStd(BaseModel):
    expressiveness: Literal["flat", "slightly_expressive", "expressive", "highly_expressive"] = (
        Field(
            ...,
            description=(
...
class WPS(BaseModel):
    speaking_rate: Literal["very_slow", "slow", "normal", "fast", "very_fast"] = Field(
        ...,
        description=(
...
class FillerPercentageIncrease(BaseModel):
    filler_percentage_level: Literal["normal", "abnormally high"] = Field(
        ...,
...
class PausePercentageIncrease(BaseModel):
    pause_percentage_level: Literal["normal", "abnormally high"] = Field(
