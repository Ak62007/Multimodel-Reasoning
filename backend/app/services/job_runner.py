...
def _save_segments_and_report(
    ...
    *,
    ...
    """Persist per-window reports + final report as JSON next to the master parquet."""
...
def _build_markdown(final: FinalReport) -> str:
    return (
        ...
        f"{final.behavioral_strengths}\n\n"
        "# Major Problems & Triggers\n\n"
        f"{final.vulnerabilities_and_triggers}\n\n"
        "# How to Improve\n\n"
        f"{final.areas_for_improvement}\n"
    )


def _run_agents_sync(
    master_df: pd.DataFrame, speaker_label: str
    ...
    return asyncio.run(build_report(master_df, speaker_label=speaker_label))
...
def run_job_blocking(job_id: str, settings: Settings) -> None:
    ...
    try:
        ...
        reports, final = _run_agents_sync(master_df, speaker_label)
        ...
        _save_segments_and_report(
            master_df, reports, final, segments_path=segments_path, report_path=report_path
        )
